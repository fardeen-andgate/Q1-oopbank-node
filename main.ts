...
import { Customer, BankAcc, Bank } from "./classes/class.js";
...
async function bankService(bank: Bank) {
  do {
    ...
    if (service.select === "View Balance") {
      ...
      let account = fardeenBankltd.account.find(
        (acc) => acc.accNumber == res.num
      );
      if (!account) {
        console.log(chalk.redBright.bold("Invalid Account Number"));
      }
      if (account) {
        let details = fardeenBankltd.customer.find(
          (val) => val.accNumber == res.num
        );
        console.log(
          `Dear, ${chalk.greenBright.bold(
            details?.firstName
          )} ${chalk.greenBright.bold(
            details?.lastName
          )} Your Account Balance is ${chalk.blueBright(`$${account.balance}`)}`
        );
      }
    }
    ...
    if (service.select === "Cash Withdraw") {
      ...
      if (account) {
        ...
        console.log(
          `Your remaining balance is ${chalk.cyanBright(`$${newBalance}`)}`
        );
      }
    }
    ...
    if (service.select === "Cash Deposit") {
      ...
      if (account) {
        ...
        console.log(
          `Thank you for despositing, your new balance is ${chalk.cyanBright(
            `$${newBalance}`
          )}`
        );
      }
